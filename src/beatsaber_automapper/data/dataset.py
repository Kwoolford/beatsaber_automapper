...
Provides datasets for each training stage:
    ...
    - SequenceDataset: Audio frames + onset times + token sequences (Stage 2, V5)
    - SwingSequenceDataset: Per-song swing-event windows + saber state (Stage 2, V6)
...
Preprocessed .pt format (one per song):
    {"song_id": str, "bpm": float, "mel_spectrogram": Tensor[n_mels, n_frames],
     "difficulties": {"Expert": {"onset_frames": Tensor[n_onsets],
       ...
       "swing_tokens": list[int]}},           # ← V6 addition
...
# ---------------------------------------------------------------------------
# V6 SwingSequenceDataset — windowed per-hand swing-event stream
# ---------------------------------------------------------------------------
...
def _beats_to_frame(beat: float, bpm: float) -> int:
    ...
    return int(beat * _FRAMES_PER_SEC * 60.0 / bpm)


class SwingSequenceDataset(Dataset):
    """Dataset for Stage 2 (V6) swing-event sequence training.

    Yields sliding windows over the full-song swing-event token stream stored
    in V6-format .pt files (``"swing_tokens"`` key per difficulty). Each sample
    contains:

        tokens        [max_swing_len]           — padded flat token window
        token_length  scalar                     — actual token count
        saber_state   [window_events, 12]        — physical state per event
        mel           [n_mels, context_frames]   — audio context (centre of window)
        phrase_mel    [n_mels, phrase_frames]    — wide audio context (phrase emb)
        structure     [8, context_frames]        — per-frame structure features
        difficulty    scalar
        genre         scalar
        mapper_id     scalar

    Requires ``.pt`` files to contain ``swing_tokens`` in each difficulty dict.
    Use ``scripts/convert_to_swing.py`` to add V6 data to existing V5 files.
    """
    ...
    def __init__(
        ...
        data_dir: Path | str,
        split: str = "train",
        window_events: int = 128,
        window_hop: int = 64,
        max_swing_len: int = 512,
        ...
        """Initialize SwingSequenceDataset.

        Args:
            data_dir: Directory with V6-format .pt files and splits.json.
            split: One of "train", "val", "test".
            window_events: Number of swing events per training sample.
            window_hop: Event stride between consecutive windows.
            max_swing_len: Max token length after padding (tokens, not events).
            context_frames: Mel frames for the per-sample audio context window.
            phrase_frames: Mel frames for the phrase-level audio context window.
            difficulties: Which difficulties to include. None = all available.
            exclude_categories: Mod categories to exclude.
            mapper_id: Integer cohort ID to assign to all samples (0 = unknown).
            mirror_augment: If True, 50% of training samples are L↔R mirrored.
        """
        self.data_dir = Path(data_dir)
        self.window_events = window_events
        self.window_hop = window_hop
        self.max_swing_len = max_swing_len
        self.context_frames = context_frames
        self.phrase_frames = phrase_frames
        self.mapper_id = mapper_id
        self.mirror_augment = mirror_augment
        self.target_difficulties = difficulties
        self.exclude_categories = set(exclude_categories) if exclude_categories else set()

        # Load split / blacklist / whitelist
        splits_path = self.data_dir / "splits.json"
        if splits_path.exists():
            with open(splits_path) as f:
                song_ids: set[str] | None = set(json.load(f).get(split, []))
        else:
            song_ids = None
        ...
        whitelist: set[str] | None = None
        if (wp := self.data_dir / "whitelist.json").exists():
            with open(wp) as f:
                whitelist = set(json.load(f).keys())

        # Index: (pt_path, diff_name, event_start, diff_id, genre_idx)
        self.samples: list[tuple[Path, str, int, int, int]] = []
        for pt_path in sorted(self.data_dir.glob("*.pt")):
            ...
            # Peek at metadata without fully loading the tensor file
            try:
            ...
            for diff_name, diff_data in data.get("difficulties", {}).items():
                if self.target_difficulties and diff_name not in self.target_difficulties:
                    continue
                swing_tokens = diff_data.get("swing_tokens")
                if not swing_tokens:
                    continue  # V5-only file — needs conversion

                # Count events in this stream
                n_events = self._count_events(swing_tokens)
                if n_events < window_events:
                    continue  # song too short for even one window

                diff_id = DIFFICULTY_MAP.get(diff_name, 3)
                # Slide windows
                start = 0
                while start + window_events <= n_events:
                    self.samples.append((pt_path, diff_name, start, diff_id, genre_idx))
                    start += window_hop
                # Include a final tail window if needed
                if n_events > window_events and (n_events - window_events) % window_hop != 0:
                    self.samples.append(
                        (pt_path, diff_name, n_events - window_events, diff_id, genre_idx)
                    )
    ...
    @staticmethod
    def _count_events(swing_tokens: list[int]) -> int:
        """Count the number of events in a swing token stream."""
        ...
        count = 0
        for tok in swing_tokens:
            if tok in (HAND_LEFT, HAND_RIGHT, HAND_NONE):
                count += 1
        return count
    ...
    def __getitem__(self, idx: int) -> dict[str, torch.Tensor]:
        ...
        pt_path, diff_name, event_start, diff_id, genre_idx = self.samples[idx]
        data = self._load(pt_path)
        mel_full = data["mel_spectrogram"]           # [n_mels, T]
        bpm = float(data.get("bpm", 120.0))
        diff_data = data["difficulties"][diff_name]
        swing_tokens: list[int] = diff_data["swing_tokens"]

        tok = SwingEventTokenizer()
        all_events = tok.decode_events(swing_tokens)

        # Slice the event window
        event_end = min(event_start + self.window_events, len(all_events))
        window_evts = all_events[event_start:event_end]

        # Optionally mirror (swap L↔R, mirror X and directions)
        do_mirror = self.mirror_augment and random.random() < 0.5

        # Compute saber states for the window — state at position i uses events BEFORE it.
        # Pass all prior events so the state correctly reflects the full song history.
        prior_states = compute_saber_states(all_events[:event_start + len(window_evts)])
        window_states = prior_states[event_start:event_start + len(window_evts)]  # [W, 12]

        if do_mirror:
            window_states = _mirror_saber_state(window_states)

        # Build the token subsequence for this window:
        # Re-encode just the window events as a mini stream (BOS … EOS)
        window_tokens = self._events_to_tokens(window_evts, tok, do_mirror=do_mirror)

        # Pad/truncate tokens
        if len(window_tokens) > self.max_swing_len:
            window_tokens = window_tokens[: self.max_swing_len]
        token_length = len(window_tokens)
        padded = window_tokens + [PAD] * (self.max_swing_len - len(window_tokens))
        ...
        for tok_pos, tok in enumerate(padded):
            if tok in (HAND_LEFT, HAND_RIGHT, HAND_NONE):
                event_idx += 1
            effective_idx = max(0, min(event_idx, n_events - 1)) if n_events > 0 else 0
            if n_events > 0:
                saber_state_per_token[tok_pos] = window_states[effective_idx]

        # Audio context: centred on the middle of the window
        mid_beat = (window_evts[0].beat + window_evts[-1].beat) / 2.0 if window_evts else 0.0
        center_frame = _beats_to_frame(mid_beat, bpm)
        ...
        if structure is not None:
            structure = _pad_structure(structure)
            struct_win = self._extract_mel_window(
                structure, center_frame, self.context_frames, n_frames
            )
        else:
        ...
        return {
            ...
            "token_length": torch.tensor(token_length, dtype=torch.long),
            "saber_state": saber_state_per_token,
            "mel": mel_window,
            "phrase_mel": phrase_mel,
            "structure": struct_win,
            "difficulty": torch.tensor(diff_id, dtype=torch.long),
            "genre": torch.tensor(genre_idx, dtype=torch.long),
    ...
    @staticmethod
    def _extract_mel_window(
        mel: torch.Tensor, center: int, width: int, n_frames: int,
    ) -> torch.Tensor:
        half = width // 2
        ...
        if window.shape[1] < width:
            window = torch.nn.functional.pad(window, (0, width - window.shape[1]))
        return window

    @staticmethod
    def _events_to_tokens(events: list, tok: object, *, do_mirror: bool) -> list[int]:
        """Encode a list of _SwingEvent objects into a token list."""
        from beatsaber_automapper.data.swing_tokenizer import (
            _DT_BINS,
            ANGLE_BASE,
            ANGLE_COUNT,
        ...
        # Direction mirror: 2↔3, 4↔5, 6↔7
        dir_mirror = {0: 0, 1: 1, 2: 3, 3: 2, 4: 5, 5: 4, 6: 7, 7: 6, 8: 8}
        # Column mirror: 0↔3, 1↔2
        col_mirror = {0: 3, 1: 2, 2: 1, 3: 0}
        ...
        for evt in events:
            ...
            hand = hand_mirror[evt.hand] if do_mirror else evt.hand
            x = col_mirror[evt.x] if do_mirror else evt.x
            direction = dir_mirror[evt.direction] if do_mirror else evt.direction

            tokens.append(hand)
            tokens.append(DT_BASE + dt_bin)
            tokens.append(evt.kind)
            tokens.append(X_BASE + x)
            tokens.append(Y_BASE + evt.y)

            if evt.kind == BOMB:
                pass
            elif evt.kind == CHAIN_TAIL:
                tokens.append(SQUISH_BASE + min(evt.field_d, SQUISH_COUNT - 1))
            else:
                tokens.append(DIR_BASE + direction)
                if evt.kind == NOTE:
                    fd = evt.field_d
                    if do_mirror:
                        # Mirror angle: negate (bins are symmetric around centre=3)
                        fd = ANGLE_COUNT - 1 - fd
                    tokens.append(ANGLE_BASE + min(fd, ANGLE_COUNT - 1))
                elif evt.kind in (ARC_HEAD, ARC_TAIL):
                    tokens.append(MU_BASE + min(evt.field_d, MU_COUNT - 1))
                else:  # CHAIN_HEAD
                    tokens.append(SLICE_BASE + min(evt.field_d, SLICE_COUNT - 1))

        tokens.append(EOS)
        return tokens


def _mirror_saber_state(state: torch.Tensor) -> torch.Tensor:
    """Mirror a saber-state tensor: swap L↔R hands, flip X positions and dx."""
    # Columns: 0=L_x 1=L_y 2=L_dx 3=L_dy 4=L_dt 5=L_parity
    #          6=R_x 7=R_y 8=R_dx 9=R_dy 10=R_dt 11=R_parity
    mirrored = state.clone()
    # Swap L and R halves
    mirrored[:, :6] = state[:, 6:]
    mirrored[:, 6:] = state[:, :6]
    # Flip X normalised positions (x/3 → (3-x)/3 = 1 - x/3)
    mirrored[:, 0] = 1.0 - mirrored[:, 0]
    mirrored[:, 6] = 1.0 - mirrored[:, 6]
    # Flip horizontal direction component (dx → -dx)
    mirrored[:, 2] = -mirrored[:, 2]
    mirrored[:, 8] = -mirrored[:, 8]
    return mirrored


def swing_collate_fn(batch: list[dict[str, torch.Tensor]]) -> dict[str, torch.Tensor]:
    """Collate function for SwingSequenceDataset.

    Stacks all fixed-size tensors. ``saber_state`` is already padded to
    ``window_events`` rows per sample, so stacking is straightforward.
    """
    keys = batch[0].keys()
