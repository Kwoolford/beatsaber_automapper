#!/usr/bin/env python
"""Calibrate the rhythm/beat-grid human reference (eval suite v2, axis A2).

Computes the raw rhythm metrics over N human Standard maps from data/raw and writes
...
from beatsaber_automapper.evaluation import rhythm  # noqa: E402
...
def load_human(zip_path: pathlib.Path, difficulty: str = "Expert"):
    ...
    tmp = pathlib.Path(tempfile.mkdtemp(prefix="rhythm_calib_"))
    try:
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(rhythm.REFERENCE_PATH))
    a = ap.parse_args()
