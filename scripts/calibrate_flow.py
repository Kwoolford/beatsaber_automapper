#!/usr/bin/env python
"""Calibrate the flow/ergonomics human reference (eval suite v2, axis A1).

Computes the raw flow metrics over N human Standard maps from data/raw and writes
the median/MAD reference that `evaluation.flow.score_against_reference` scores
...
Usage:
  python scripts/calibrate_flow.py --n 200
"""
...
from beatsaber_automapper.evaluation import flow  # noqa: E402
...
def load_human(zip_path: pathlib.Path, difficulty: str = "Expert"):
    ...
    tmp = pathlib.Path(tempfile.mkdtemp(prefix="flow_calib_"))
    try:
...
def main() -> None:
    ...
    ap.add_argument("--out", default=str(flow.REFERENCE_PATH))
    a = ap.parse_args()
