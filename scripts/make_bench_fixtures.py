...
# song id -> the difficulty to KEEP as the fixture's map (the reference stays data/raw/<id>.zip,
# where load_map prefers ExpertStandard).
DIFFICULTY_CONTROL = {"1f333": "expertplusstandard.dat", "1f8d6": "expertplusstandard.dat"}


def difficulty_control(sid: str, keep: str) -> pathlib.Path:
    src = REPO / "data" / "raw" / f"{sid}.zip"
    dst = OUT / f"HUMANPLUS__{sid}.zip"
...
def main() -> int:
    for sid, keep in DIFFICULTY_CONTROL.items():
        p = difficulty_control(sid, keep)
        print(f"{p.relative_to(REPO)}  {p.stat().st_size / 1024:.0f} KB  "
              f"({zipfile.ZipFile(p).namelist()})")
    print("\nreference for both: data/raw/<sid>.zip (Expert, same mapper) -- "
          "run with `--vs <sid>`; a query that fires here is reading DIFFICULTY, not a defect.")
    return 0
